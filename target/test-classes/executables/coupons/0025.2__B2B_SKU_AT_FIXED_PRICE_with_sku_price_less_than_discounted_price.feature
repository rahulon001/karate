Feature: B2B_SKU_AT_FIXED_PRICE checkout cancel flow __0025.2
  ...
  Background:
    ...
    * def sku_price = apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].less_price
    ...
    * assert skuID.indexOf("811143212226") == -1
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE checkout checkoutCancel flow __0025.2
    ...
    * def b2b_coupons = db.readRows("SELECT * FROM B2B_COUPON WHERE ID IN (\'" +coupon_code_status[0]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[1]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[2]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[3]["B2B_COUPON_ID"]+ "\')")
    * print b2b_coupons
    ...
    * def b2b_coupon_redemption = db.readRows("SELECT * FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (\'" +coupon_code_status[0]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[1]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[2]["B2B_COUPON_ID"]+ "\',\'" +coupon_code_status[3]["B2B_COUPON_ID"]+ "\')")
 ...
 #   * match parseInt(redeem_quantity_811143212226_c[0].QUANTITY) == parseInt(redeem_quantity_811143212226[0].QUANTITY + 2)
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE checkout checkoutCancel flow __0025.2
    ...
    * def body =
    """
      ...
      "couponCodes":[#(couponCode3), #(couponCode4), #(couponCode5), #(couponCode6)],
      "skuData": #(apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption__0025"].skuData)
      }
    """
   * set body.skuData[*].skuPrice =  apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].less_price
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE delayed cancel checkoutCancel flow __0025.2
      # redemption count
    * def redeem_quantity_811143212226 = db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212226+ "\')) and masid=\'" +masid+ "\'")
    * def redeem_quantity_811143212227 = db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212227+ "\')) and masid=\'" +masid+ "\'")
    * def redeem_quantity_811143212228 = db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212228+ "\')) and masid=\'" +masid+ "\'")
  ...
  #  * def redeem_quantity_811143212226_r = db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212226+ "\')) and masid=\'" +masid+ "\'")
    ...
    * def redeem_count_811143212228_r = db.readRows("SELECT REDEEM_COUNT FROM B2B_COUPON WHERE ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212228+ "\'))")
    ...
    # redemption count
 #   * match parseInt(redeem_quantity_811143212226_r[0].QUANTITY) == parseInt(redeem_quantity_811143212226[0].QUANTITY)
 ...
 #   * match parseInt(redeem_count_811143212226_r[0].REDEEM_COUNT) == parseInt(redeem_count_811143212226[0].REDEEM_COUNT)
  ...
  Scenario: Switching off the features __0025.2.
