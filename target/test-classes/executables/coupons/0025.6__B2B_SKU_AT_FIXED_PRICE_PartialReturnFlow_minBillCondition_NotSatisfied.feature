...
#    * assert result.response["skuCoupons"][skuID_index_811143212228]["coupons"][0].discount == apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].discount_811143212228
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE checkout partial return flow __0025.6.
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\',\'" +couponCode2+ "\',\'" +couponCode4+ "\',\'" +couponCode5+ "\',\'" +couponCode6+ "\')")
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE checkout partial return flow __0025.6.
    ...
    * def body =
    """
      ...
      "couponCodes":[#(couponCode1), #(couponCode2), #(couponCode4), #(couponCode5), #(couponCode6)],
    ...
    * match response.discountAmount == apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].discountAmount_mbv
    * match response.newBillAmount == apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].newBillAmount_mbv
...
#    * assert response.data[coupon_code.indexOf(couponCode3)].discount == apiComponents["B2B_SKU_AT_FIXED_PRICE_coupon_redemption_responseData__0025"].discount_811143212228
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE delayed cancel partial return flow __0025.6
    ...
    * def update_delivered_TS = db.updateRow("UPDATE FC_MERCHANT_COUPONS SET DELIVERED_TS = CURRENT_TIMESTAMP - 20 WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode4+ "\',\'" +couponCode5+ "\',\'" +couponCode6+ "\')")
 ...
 #   * match parseInt(redeem_count_811143212228_r[0].REDEEM_COUNT) == parseInt(redeem_count_811143212228[0].REDEEM_COUNT)
  ...
  Scenario: B2B_SKU_AT_FIXED_PRICE redeem partial return flow __0025.6
    * def body  =
    ...
    When method post
    Then status 200
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode4+ "\',\'" +couponCode5+ "\',\'" +couponCode6+ "\')")
    * print coupon_code_status
