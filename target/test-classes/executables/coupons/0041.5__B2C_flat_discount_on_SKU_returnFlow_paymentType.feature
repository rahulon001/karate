Feature: B2C flat discount on SKU payment type return flow __0041.5
  ...
  Background:
    ...
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2C_flat_discount_on_SKU_payment")
  ...
  Scenario: merchant verify  B2C flat discount on SKU payment type __0041.5.
    * def body =
    """
      {
        ...
        "tId": "xyzklj199",
        "billAmount": #(apiComponents["B2C_flat_discount_on_SKU_paymentType__0041.5"].billAmount),
        "couponCodes": "#(couponCode1)",
        "skuData": #(apiComponents["B2C_flat_discount_on_SKU_paymentType__0041.5"].skuData)
      }
    """
    ...
    When method post
    Then status 200
    * match response.data[0].discountType == apiComponents["B2C_flat_discount_on_SKU_paymentType_responseData__0041.5"].discountType1
    * match response.data[0].discount == apiComponents["B2C_flat_discount_on_SKU_paymentType_responseData__0041.5"].discount1
    * match response.discountedItems[0].skuId == apiComponents["B2C_flat_discount_on_SKU_paymentType_responseData__0041.5"].discountedItems_skuId1
    * match response.newBillAmount == apiComponents["B2C_flat_discount_on_SKU_paymentType_responseData__0041.5"].newBillAmount
  ...
  Scenario: merchant return-order b2c B2C flat discount on SKU payment type __0041.5.
    ...
    * def body  =
    """
    {
      ...
      "originalCart": #(apiComponents["B2C_flat_discount_on_SKU_paymentType__0041.5"].skuData),
      "returnedItems": #(apiComponents["B2C_flat_discount_on_SKU_paymentType__0041.5"].skuData)
    }
    """
