Feature: BxGy B2B mark redeem flow parent __0001.2
  ...
  Background:
    ...
    * def b2c_coupon_redemption = callonce read("0001__B2C_BxGy_parent_mark_redeem_flow_discountType.feature@bxgy_parent")
    * json result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_parent_cart_redemption") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code getEligible __0001.2
  ...
  Scenario: B2B coupon code release getEligible __0001.2
    ...
    * json result = call read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_parent_cart_redemption") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0001.2
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_BxGy_parent_discountType__0001.2"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_BxGy_parent_discountType__0001.2"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_BxGy_parent_responseData__0001.2"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_BxGy_parent_responseData__0001.2"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
    * print "<====== coupon_code_status ======>", coupon_code_status
   ...
   Scenario: B2B coupon code deliver __0001.2.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code mark redeem __0001.2.
    * def body  =
    """
    {
    }
