Feature: BxGy B2B mark redeem flow parent flat __0009.2
  ...
  Background:
    ...
    * def b2c_discount_coupon_redemption = callonce read("0009__B2C_BxGy_parent_flat_discount_mark_redeem_flow_discount_type.feature")
    * json result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_flat_parent_cart_redemption") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code getEligible __0009.2
  ...
  Scenario: B2B coupon code release getEligible __0009.2
    ...
    * json result = call read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_flat_parent_cart_redemption") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0009.2
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_BxGy_parent_flat_discountType__0009.2"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_BxGy_parent_flat_discountType__0009.2"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_BxGy_parent_flat_responseData__0009.2"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_BxGy_parent_flat_responseData__0009.2"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
    * print "<====== coupon_code_status ======>", coupon_code_status
   ...
   Scenario: B2B coupon code deliver __0009.2.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code mark redeem __0009.2.
    * def body  =
    """
    {
    }
