...
Feature: B2B_FLAT_DISCOUNT_ON_SKU MarkRedemptionFlow __0036.7
  ...
  Background:
    ...
    * def transaction_id = callonce transactionID
    ...
    # creation of the sku ids
    * def sku_id_1 = callonce skuID_1
    * def sku_id_2 = callonce skuID_2

    # creation of b2b coupon
    * def helperMethods = Java.type('executables.utils.HelperMethods')
    * def b2b_bulk_upload_files = call helperMethods.B2B_coupon_BulkUpload(sku_id_1,sku_id_2,)
  ...
  Scenario: B2B_FLAT_DISCOUNT_ON_SKU checkout Mark Redemption Flow __0036.
    * print "0000000000000000000000000000", sku_id_1
    * print "1111111111111111111111111111", sku_id_2
    ...
    * def schema = schemaAndValidation["checkout_FC_merchant_coupon"]
    ...
    # redemption count
    * assert db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212221+ "\'))")[0].QUANTITY == redeem_quantity_811143212221[0].QUANTITY + 1
    ...
    * assert db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +811143212222+ "\'))")[0].QUANTITY == redeem_quantity_811143212222[0].QUANTITY + 2
