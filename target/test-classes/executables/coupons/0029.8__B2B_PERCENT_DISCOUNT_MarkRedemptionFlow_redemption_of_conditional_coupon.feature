Feature: Perform mark redeem for conditional coupon generated and redemption via b2b coupon.
  ...
  Background:
    ...
    * def result0 = callonce read("0029__B2B_PERCENT_DISCOUNT_MarkRedemptionFlow.feature")
...
#    * def b2b_coupons = db.readRows("SELECT * FROM B2B_COUPON WHERE ID IN (\'" +coupon_code_status[0]["B2B_COUPON_ID"]+ "\')")
#    * print b2b_coupons
#    * def update_conditional_coupon_status = db.updateRow("UPDATE B2B_COUPON SET status = "16" WHERE SKU_ID IN (\'" +b2b_coupons[0]["SKU_ID"]+ "\')")
