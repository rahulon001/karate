Feature: B2C flat discount on SKU B2B mark redeem flow  __0041.10
  ...
  Background:
    ...
    * def b2c_discount_coupon_redemption = callonce read("0041.6__B2C_flat_discount_on_SKU_mark_redeem_flow_discountType_BillCondition_Satisfied.feature@B2C_flat_discount_on_SKU_BillCondition")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_B2C_flat_discount_on_SKU_redemption_BillCondition") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0041.10
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_flat_discount_on_SKU_discountType__0041.10"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_flat_discount_on_SKU_discountType__0041.10"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_flat_discount_on_SKU_discountType_responseData__0041.10"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_flat_discount_on_SKU_discountType_responseData__0041.10"].newBillAmount
  ...
  Scenario: B2B coupon code deliver __0041.10.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code delayed cancel __0041.10.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
