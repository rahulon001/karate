Feature: b2c percentage discount on sku B2B mark redeem flow __0042.10
  ...
  Background:
    ...
    * def b2c_discount_coupon_redemption = callonce read("0042.6__B2C_percentage_discount_on_SKU_mark_redeem_flow_discountType_BillCondition_Satisfied.feature@B2C_percentage_discount_on_SKU_BillCondition")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_B2C_percentage_discount_on_SKU_redemption_BillCondition") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0042.10
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_percentage_discount_on_SKU_discountType__0042.10"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_percentage_discount_on_SKU_discountType__0042.10"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_percentage_discount_on_SKU_discountType_responseData__0042.10"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_percentage_discount_on_SKU_discountType_responseData__0042.10"].newBillAmount
  ...
  Scenario: B2B coupon code deliver __0042.10.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code delayed cancel __0042.10.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
