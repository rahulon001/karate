...
Feature: B2C_flat_discount_on_SKU mark redeem flow __0041.11
  ...
  Background:
    ...
    * def sku_price = apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].less_price
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2C_flat_discount_on_SKU_lessprice") {requestHeader: #(headerJson) , skuPrice: #(sku_price)}
  ...
  Scenario: merchant verify b2c B2C flat discount on SKU_0041
    * def body =
    """
      {
        "masId": "#(masid)",
        ...
        "tId": "xyzklj199",
        "billAmount": #(apiComponents["B2C_flat_discount_on_SKU_discountType__0041"].billAmount),
        "couponCodes": "#(couponCode1)",
        "skuData": #(apiComponents["B2C_flat_discount_on_SKU_discountType__0041"].skuData)
      }
    """
    ...
    When method post
    Then status 200
    * match response.data[0].discountType == apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].discountType1
    * match response.data[0].discount == apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].less_price_discount
    * match response.discountedItems[0].skuId == apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].discountedItems_skuId1
    * match response.discountedItems[0].skuPrice == apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].less_price_skuPrice
    * match response.newBillAmount == apiComponents["B2C_flat_discount_on_SKU_discountType_responseData__0041"].less_price_newBillAmount
  ...
  Scenario: merchant checkout b2c B2C flat discount on SKU_0041
    * def body =
    """
      {
        "masId": "#(masid)",
  ...
  Scenario: merchant accept-order b2c B2C flat discount on SKU_0041
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: merchant delivered-order b2c B2C flat discount on SKU_0041
    * def body =
    """
    {
      "couponCodes":[#(couponCode1)],
