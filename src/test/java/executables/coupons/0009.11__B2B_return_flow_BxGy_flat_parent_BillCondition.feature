Feature: BxGy B2B mark redeem flow parent __0009.3
  ...
  Background:
    ...
    * def result = callonce read("0009.7__B2C_BxGy_parent_flat_discount_mark_redeem_flow_discount_type_minBillCondition_Satisfied.feature")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_flat_parent_cart_BillCondition") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code getElogible __0009.3
  ...
  Scenario: B2B coupon code checkout __0009.3
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_BxGy_parent_flat_discountType__0009.11"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_BxGy_parent_flat_discountType__0009.11"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_BxGy_parent_flat_responseData__0009.11"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_BxGy_parent_flat_responseData__0009.11"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
    * print "<====== coupon_code_status ======>", coupon_code_status
   ...
   Scenario: B2B coupon code deliver __0009.3.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code delayed cancel __0009.3.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
