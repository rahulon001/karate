Feature: Perform mark redeem for conditional coupon generated and redemption via b2b coupon.
  ...
  Background:
    ...
    * def result0 = callonce read("0025__B2B_SKU_AT_FIXED_PRICE_MarkRedemptionFlow.feature")
...
#    * def b2b_coupons = db.readRows("SELECT * FROM B2B_COUPON WHERE ID IN (\'" +coupon_code_status[0]["B2B_COUPON_ID"]+ "\')")
#    * print b2b_coupons
#    * def update_conditional_coupon_status = db.updateRow("UPDATE B2B_COUPON SET SKU_ID = \'122326\' WHERE ID IN (\'" +b2b_coupons[0]["ID"]+ "\')")
