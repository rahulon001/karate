...
Feature: b2c percentage discount on sku mark redeem flow __0042.6
  ...
  Background:
    ...
    * def bill_Amount = apiComponents["B2C_percentage_discount_on_SKU_discountType__0042.6"].billAmount
  #  * print #(bill_Amount)
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2C_percentage_discount_on_SKU_BillCondition") {requestHeader: #(headerJson) , billAmount: #(bill_Amount)}
    * assert bill_Amount>=apiComponents["B2C_percentage_discount_on_SKU_discountType_responseData__0042.6"].minbillCondition
  ...
  Scenario: merchant verify b2c b2c percentage discount on sku __0042.6.
    * def body =
    """
      {
        "masId": "#(masid)",
  ...
  Scenario: merchant checkout b2c b2c percentage discount on sku __0042.6.
    * def body =
    """
      {
        "masId": "#(masid)",
  ...
  Scenario: merchant accept-order b2c b2c percentage discount on sku __0042.6.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: merchant delivered-order b2c b2c percentage discount on sku __0042.6.
    * def body =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: merchant mark redeem b2c b2c percentage discount on sku __0042.6.
    * def body  =
    """
    {
