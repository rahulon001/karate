@conditionalCoupon
Feature: Perform B2B conditional coupon mark redeeming flow MAS.
  ...
  Background:
    ...
    * def result0 = callonce read("B2B_conditionalCoupon_MarkRedemptionFlow_MAS_merchantGroup.feature@conditional_coupon_mark_redeem_MAS")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_conditional_coupon_redeeming_MAS") {requestHeader: #(headerJson)}
  ...
  Scenario: merchant checkout B2BMarkRedemptionFlow.
    ...
    * def b2b_conditional_coupons = db.readRows("SELECT * FROM B2B_COUPON WHERE DERIVED_FROM IN (SELECT CONDITIONAL_COUPON_ID FROM B2B_COUPON WHERE ID IN (\'" +coupon_code_status[0]["B2B_COUPON_ID"]+ "\'))")
  ...
  Scenario: Switching off the features 1.
