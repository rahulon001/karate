Feature: BxGy B2B mark redeem flow parent __0008.3
  ...
  Background:
    ...
    * def b2c_discount_coupon_redemption = callonce read("0008.7__B2C_BxGy_child_percentage_off_mark_redeem_flow_discount_type_BillCondition_Satisfied.feature")
#    * def result = callonce read("0016__B2C_BxGy_parent_mark_redeem_flow_discountType.feature")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_percentage_child_cart_redemption_BillCondition") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code getEligible __0008.3
  ...
  Scenario: B2B coupon code checkout __0008.3
    * header override_flag = redeem_flags
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_BxGy_child_percentage_discountType__0008.11"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_BxGy_child_percentage_discountType__0008.11"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_BxGy_child_percentage_responseData__0008.11"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_BxGy_child_percentage_responseData__0008.11"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
    * print "<====== coupon_code_status ======>", coupon_code_status
   ...
   Scenario: B2B coupon code deliver __0008.3.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
  ...
  Scenario: B2B coupon code delayed cancel __0008.3.
    * def body  =
    """
    {
      "couponCodes":[#(couponCode1)],
