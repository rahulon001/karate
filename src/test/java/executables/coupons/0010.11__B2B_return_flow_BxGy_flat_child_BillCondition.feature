Feature: BxGy B2B mark redeem flow parent __0010.3
  ...
  Background:
    ...
    * def result = callonce read("0010.7__B2C_BxGy_child_flat_discount_mark_redeem_flow_discount_type_minBillCondition_Satisfied.feature")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_BxGy_flat_child_cart_redemption_BillCondition") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0010.3
    ...
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_BxGy_child_flat_discountType__0010.11"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_BxGy_child_flat_discountType__0010.11"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_BxGy_child_flat_responseData__0010.11"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_BxGy_child_flat_responseData__0010.11"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
    * print coupon_code_status
