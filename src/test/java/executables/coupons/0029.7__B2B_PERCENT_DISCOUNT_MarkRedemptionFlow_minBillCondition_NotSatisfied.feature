Feature: B2B_PERCENT_DISCOUNT MarkRedemptionFlow __0029.7
  ...
  Background:
    ...
    * def bill_amount = apiComponents["B2B_PERCENT_DISCOUNT_coupon_redemption_responseData__0029"].billAmount_mbv
...
#    * assert result.response["skuCoupons"][skuID_index_711143212228]["coupons"][0].discount == apiComponents["B2B_PERCENT_DISCOUNT_coupon_redemption_responseData__0029"].discount_711143212228
  ...
  Scenario: B2B_PERCENT_DISCOUNT checkout Mark Redemption Flow __0029.7.
...
##    * def redeem_quantity_711143212228 = db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +711143212228+ "\'))")
    ...
    * def body =
    """
      {
      "masId": "#(masid)",
      "billAmount": #(apiComponents["B2B_PERCENT_DISCOUNT_coupon_redemption_responseData__0029"].billAmount_mbv),
    ...
    * assert response.discountAmount == apiComponents["B2B_PERCENT_DISCOUNT_coupon_redemption_responseData__0029"].discountAmount_mbv
    * assert response.newBillAmount == apiComponents["B2B_PERCENT_DISCOUNT_coupon_redemption_responseData__0029"].newBillAmount_mbv
...
#
 ...
 #   * match parseInt(redeem_quantity_711143212226_c[0].QUANTITY) == parseInt(redeem_quantity_711143212226[0].QUANTITY + 2)
 #   * match parseInt(redeem_quantity_711143212227_c[0].QUANTITY) == parseInt(redeem_quantity_711143212227[0].QUANTITY + 3)
##    * assert db.readRows("SELECT QUANTITY FROM B2B_COUPON_REDEMPTION WHERE B2B_COUPON_ID IN (SELECT ID FROM B2B_COUPON bbc WHERE SKU_ID IN (\'" +711143212228+ "\'))")[0].QUANTITY == redeem_quantity_711143212228[0].QUANTITY + 1

  Scenario: B2B_PERCENT_DISCOUNT delivered Mark Redemption Flow __0029.7.
    ...
    * def update_delivered_TS = db.updateRow("UPDATE FC_MERCHANT_COUPONS SET DELIVERED_TS = CURRENT_TIMESTAMP - 20 WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\',\'" +couponCode2+ "\',\'" +couponCode4+ "\',\'" +couponCode5+ "\',\'" +couponCode6+ "\')")
    * def schema = schemaAndValidation["delivered_FC_merchant_coupon"]
    * match each coupon_code_status == schema

  Scenario: B2B_PERCENT_DISCOUNT redeem Mark Redemption Flow __0029.7.
    * def body  =
    """
    {
  ...
  Scenario: Switching off the features __0029.7.
