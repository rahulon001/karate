Feature: B2C flat discount on SKU B2B mark redeem flow  __0041.10
  ...
  Background:
    ...
    * def b2c_discount_coupon_redemption = callonce read("0041.17__B2C_flat_discount_on_SKU_markRedeem_multiredeem_SKU_Upload_OR.feature")
    * def result = callonce read("B2C_B2B_CouponCodeGeneration.feature@B2B_flat_discount_on_SKU_redemption_SKU_Upload_OR") {requestHeader: #(headerJson)}
  ...
  Scenario: B2B coupon code checkout __0041.10
    ...
    * def body =
    """
        {
          "masId": "#(masid)",
          "billAmount": #(apiComponents["B2B_flat_discount_on_SKU_discountType__0041.18"].billAmount),
          "transactionId": "#(transaction_id)",
          "couponCodes": [#(couponCode1)],
          "skuData": #(apiComponents["B2B_flat_discount_on_SKU_discountType__0041.18"].skuData)
        }
    """
    ...
    When method post
    Then status 200
    And match response.discountAmount == apiComponents["B2B_flat_discount_on_SKU_discountType_responseData__0041.18"].discountAmount
    And match response.newBillAmount == apiComponents["B2B_flat_discount_on_SKU_discountType_responseData__0041.18"].newBillAmount
    * def coupon_code_status = db.readRows("SELECT * FROM FC_MERCHANT_COUPONS WHERE MERCHANT_COUPON_CODE IN (\'" +couponCode1+ "\')")
